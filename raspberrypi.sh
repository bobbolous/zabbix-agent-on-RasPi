...
# modified 2024 Roy Sigurd Karlsbakk <roy@karlsbakk.net>
...
case "$1" in
        ...
        coreclock)
                ...
                vcgencmd measure_clock core | sed s/"frequency(1)="//g
                ;;
        cpuclock)
        ...
        h264clock)
                ...
                vcgencmd measure_clock h264 | sed s/"frequency(28)="//g
                ;;
        ispclock)
        ...
        uartclock)
                ...
                vcgencmd measure_clock uart | sed s/"frequency(22)="//g
                ;;
        pwmclock)
                # pwm Clock Speed in Hz
                vcgencmd measure_clock pwm | sed s/"frequency(25)="//g
                ;;
        emmcclock)
        ...
        pixelclock)
                ...
                vcgencmd measure_clock pixel | sed s/"frequency(29)="//g
                ;;
        vecclock)
                # vec Clock Speed in Hz
                vcgencmd measure_clock vec | sed s/"frequency(10)="//g
                ;;
        hdmiclock)
        ...
        dpiclock)
                ...
                vcgencmd measure_clock dpi | sed s/"frequency(4)="//g
                ;;
        cpumem)
                # CPU Memory in MByte
        ...
        cpuvoltage)
                # CPU Voltage in Volt
                vcgencmd measure_volts | tr -d "volt=V"
                ;;
        firmwareversion)
        ...
        sdramcvoltage)
                # sdram_c Voltage in Volt
                vcgencmd measure_volts sdram_c| tr -d "volt=V"
                ;;
        sdramivoltage)
                # sdram_i Voltage in Volt
                vcgencmd measure_volts sdram_i| tr -d "volt=V"
                ;;
        sdrampvoltage)
                # sdram_p Voltage in Volt
                vcgencmd measure_volts sdram_p| tr -d "volt=V"
                ;;
        temperature)
        ...
        throttlestate)
                ...
                vcgencmd get_throttled | sed s/"throttled="//g | perl -e '$number = hex(<STDIN>); printf "%.32b\n", $number'
                ;;
        *)
